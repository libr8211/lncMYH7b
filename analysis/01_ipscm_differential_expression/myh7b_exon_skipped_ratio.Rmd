...
```{r}
# let's make a plot!
g <- ggplot(myh7b_tpm_asokd, aes(x = transcript_id, y = log2(tpm)))
g + geom_point() + 
  theme_paperwhite()
...
myh7b_tpm_asokd$transcript_id <- factor(myh7b_tpm_asokd$transcript_id, levels = c("ENST00000262873.12_exon_8_skipped", "ENST00000262873.12"),
                                        labels = c("lncMYH7B", "MYH7B"))
g <- ggplot(myh7b_tpm_asokd, aes(x = transcript_id, y = tpm))
g + geom_boxplot() + 
