...
which(gtf$geneID == "ENSG00000078814.16")
...
# Export count tables
write.table(tpm_spoe, file = "../../ipscm_rnaseq/results/tpm_spoe.txt", 
            sep="\t", row.names=T, col.names=T, quote=F)
write.table(tpm_asokd, file = "../../ipscm_rnaseq/results/tpm_asokd.txt", 
            sep="\t", row.names=T, col.names=T, quote=F)
```
...
resdf_spoe <- res_shrunken_spoe %>% as.data.frame() %>%
  ...
  merge(g2s)
write_csv(resdf_spoe, "../../ipscm_rnaseq/results/res_shrunken_spoe.csv")
resdf_asokd <- res_shrunken_asokd %>% as.data.frame() %>%
  rownames_to_column("geneID") %>%
  merge(g2s)
write_csv(resdf_asokd, "../../ipscm_rnaseq/results/res_shrunken_asokd.csv")
```
...
plotDEGPCA(res_spoe, counts = rld_spoe, alpha = 0.01, lfcThreshold = 0)
plotDEGPCA(res_asokd, counts = rld_asokd, alpha = 0.01, lfcThreshold = 0)
```
...
```{r plot_deg_heatmap}
plotDEGHeatmap(res_spoe, counts = rld_spoe, clusteringMethod = "ward.D2", scale = "row", alpha = 0.01, 
               color = pals::kovesi.diverging_bwr_40_95_c42,
               legendColor = pals::watlington)

plotMA(res_spoe, lfcThreshold = params$lfc_threshold, ntop = 10, gene2symbol = g2s, alpha = 0.01)

exsk <- data.frame("geneID" = "ENSG00000078814.16.1", "geneName" = "MYH7B_exon7_skipped")
g2s <- bind_rows(g2s, exsk)
plotVolcano(res_spoe, lfcThreshold = params$lfc_threshold, ntop = 10, gene2symbol = g2s, alpha = 0.01,
            sigPointColor = c(upregulated = "#a8404c", downregulated = "#a8404c"))

plotDEGHeatmap(res_asokd, counts = rld_asokd, clusteringMethod = "ward.D2", scale = "row", alpha = 0.01, 
               color = pals::kovesi.diverging_bwr_40_95_c42,
               legendColor = pals::watlington)

plotMA(res_asokd, lfcThreshold = params$lfc_threshold, ntop = 10, gene2symbol = g2s, alpha = 0.01)

exsk <- data.frame("geneID" = "ENSG00000078814.16.1", "geneName" = "MYH7B_exon7_skipped")
g2s <- bind_rows(g2s, exsk)
plotVolcano(res_asokd, lfcThreshold = params$lfc_threshold, ntop = 10, gene2symbol = g2s, alpha = 0.01,
            sigPointColor = c(upregulated = "#a8404c", downregulated = "#a8404c"))
```
...
gene_tpm_spoe <- tpm_spoe[which(row.names(tpm_spoe) == gene_name), ] %>%
    as.data.frame() %>% rownames_to_column()
  colnames(gene_tpm_spoe) <- c("SampleID", "tpm")
  gene_tpm_spoe <- merge(gene_tpm_spoe, samples)

g <- ggplot(gene_tpm_spoe, aes(x = SampleID, y = tpm_spoe, color = condition))
g + geom_point()  + ggtitle("MYH7B expression")

gene_name <- "ENSG00000078814.16"
gene_tpm_asokd <- tpm_asokd[which(row.names(tpm_asokd) == gene_name), ] %>%
    as.data.frame() %>% rownames_to_column()
  colnames(gene_tpm_asokd) <- c("SampleID", "tpm")
  gene_tpm_asokd <- merge(gene_tpm_asokd, samples)

g <- ggplot(gene_tpm_asokd, aes(x = SampleID, y = tpm, color = condition))
g + geom_point()  + ggtitle("MYH7B expression")
...
mdout_spoe <- capture.output(res_tbl_list <- mapply(
    FUN = resultsTables,
    results = res_shrunken_spoe,
    MoreArgs = list(
        counts = dds_spoe,
